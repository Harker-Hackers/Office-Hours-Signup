...
export class UpdateHandler {
    ...
    addSocket(s: Socket, uid: string) {
        ...
        z
            ? z.set(s.id, s)
            : (this.sockets.set(uid, new Map()),
              this.sockets.get(uid)?.set(s.id, s));
        return uid;
    }
    removeSocket(uid: string, sid: string) {
        ...
        if (z) {
            let b = z.delete(sid);
            z.size == 0 && this.sockets.delete(uid);
            return b;
        }
        return false;
    }
    ...
    unfocusStudent(sid: string) {
        ...
        tid &&
            (this.student_focus.get(tid)?.delete(sid),
            this.student_focus.get(tid)?.size == 0 &&
                this.student_focus.delete(tid));
    }
    focusStudent(
        socket: Socket<
            DefaultEventsMap,
